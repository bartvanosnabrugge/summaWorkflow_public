...
#%%
# set some paths from control file
...
# 2. calculate land use fractions for MESH GRUS from CWARHM-SUMMA GRU/HRU land use counts
...
# 3. remap forcing data from grids, to MESH GRUs (CWARHM-SUMMA maps to SUMMA HRUs)
...
# 4. reindex forcing file generated by EASYMORE to match mesh drainage dbb
# first open the mf dataset with some preprocessing to set the lat and lon
# as coordinates in stead of variables.
# the preprocessing step executed by lonlatcoords is necessary to ensure
# the latitude and longitude are not appended in the time dimension
def lonlatcoords(ds):
    try:
